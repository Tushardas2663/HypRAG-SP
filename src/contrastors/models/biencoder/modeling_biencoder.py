...
class BiEncoder(PreTrainedModel):
    def __init__(self, config):
        ...
        if config.use_fused_kernels:
            ...
            if config.base_model_type == "nomic_bert":
                ...
                if model_config.model_type != "nomic_bert":
                    ...
                    config_converter = CONFIG_CONVERTER_REGISTRY.get(f"{model_config.model_type}_to_{config.base_model_type}")
                    print(f"Converting {model_config.model_type} to {config.base_model_type}")
                    model_config = config_converter(model_config)

                config.base_model_config = model_config
            ...
            elif config.base_model_type == "modernbert":
                if config.pretrained:
                    ...
                    model_config = AutoConfig.from_pretrained(config.model_name, trust_remote_code=True)
                    config.base_model_config = model_config
                    self.trunk = AutoModel.from_config(model_config)
            else:
                model_config = AutoConfig.from_pretrained(config.model_name, trust_remote_code=True)
